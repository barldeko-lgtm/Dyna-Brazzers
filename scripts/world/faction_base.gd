...
# Shared stationary base foundation. A concrete player/enemy script sets the
# faction while this class owns visuals, blocking and egg placement plumbing.
...
func _ready() -> void:
	add_to_group("faction_base")
	faction_id = CREATURE_FACTION.set_id(self, faction_id)
	configure_visual()
	world_grid = find_world_grid()

	if world_grid == null:
		push_error("FactionBase: world grid was not found for %s." % name)
		return
	...
	if not world_grid.register_blocker(self, anchor_tile, footprint_size):
		push_error(
			"FactionBase: %s cannot occupy a valid 2x2 footprint near (%d, %d)." % [
				name,
				requested_anchor.x,
				requested_anchor.y
			]
	...
	if anchor_tile != requested_anchor:
		push_warning(
			"FactionBase: %s moved from blocked anchor (%d, %d) to (%d, %d)." % [
				name,
				requested_anchor.x,
				requested_anchor.y,
				anchor_tile.x,
...
func create_faction_egg(species_data: CreatureSpeciesData) -> Node2D:
	...
	var egg_anchor := find_egg_spawn_anchor()
	...
	if eggs_container == null:
		push_error("FactionBase: Eggs container was not found for %s." % name)
		return null
	...
	CREATURE_FACTION.set_id(new_egg, faction_id)
...
func find_egg_spawn_anchor() -> Vector2i:
	...
	for candidate_anchor in preferred_anchors:
		if can_place_egg_anchor(candidate_anchor):
...
func can_place_egg_anchor(candidate_anchor: Vector2i) -> bool:
	if world_grid == null:
		return false
