...
- shared rain first snapshots the grass that already exists inside its area, then applies the cast only to that snapshot, so grass created by mature spreading always starts at stage 1 and cannot receive a second growth step from the same cast;
...
- enemy energy starting at 3000 and generated by living enemy dinosaurs; egg production spends it only after the enemy base successfully places the selected egg, while enemy rain spends it only after a valid target is found and refunds a failed application;
...
- a separate enemy spell controller listening to that four-second snapshot; below the adult-enemy-herbivore satiety threshold it limits the current rain search to the map-clipped area extending 30 tiles from every side of the 2x2 enemy base, scans only registered mature grass whose spread attempt is still available inside that contour, builds rain-center candidates only around that grass, scores each 5x5 cast by unique immediately spawnable neighbour cells inside the same contour, and spends 50 enemy energy on the best positive target;
...
- a local four-frame rain VFX; successful enemy rain additionally leaves a blue 5x5 world-space outline for four active real-time seconds, freezing its remaining lifetime while the in-game menu pauses the SceneTree, while an orange four-line contour continuously shows the current base-centred rain-search boundary;
...
- a separate compact F5 enemy-AI debug panel positioned immediately left of the right-side HUD, using font size 13 and showing AI game time, enemy energy, current production phase, herbivore cap, normalized average adult-herbivore satiety, the active hunger gate, latest action, compact population totals, and the latest enemy-rain scan size, predicted-versus-actual new-grass result, search/apply duration, session maxima, and run counts;
...
Roadmap block `0.5 — Visuals and game interface` is complete. Work on `0.6 — Carnivores and species variety` includes triceratops, tyrannosaurus, raptor, pterodactyl, egg-eater behaviour, shared carnivore/combat prototype code, and the fixed player-base foundation. Roadmap block `0.7 — Player expansion and atmosphere` includes the interactive terrain minimap, player egg creation, the first all-species flag pass, and the first gameplay-audio foundation. Roadmap block `0.8 — Enemies` has started with the two-base foundation, validated three-faction ownership, an enemy six-species catalog, separate enemy biological resources, mirrored enemy energy, four-second population snapshots, a minute-based herbivore cap, the first herbivore/predator production phases, the first automatic enemy rain decision, and the first three static enemy attack flags at the player base.

The temporary five-second round-robin producer remains disabled. Every four simulation seconds the strategic AI snapshots projected population and normalized satiety for adult enemy herbivores; eggs affect projected population but never the satiety average. Its herbivore cap is 10 through minute 10, then equals the number of completed game minutes from minute 11 onward, capped at 60. Below the cap it buys stegosaurus/triceratops eggs toward 3:1 only while adult-herbivore average satiety is at least 40%; below 40% it skips the entire turn without buying any egg. At or above the cap it ignores herbivore satiety and uses the predator task: two projected raptors first, then at least one projected tyrannosaurus, at least one projected pterodactyl, and successful tyrannosaurus/pterodactyl purchases alternating afterwards. Independently of the production phase, a below-threshold snapshot lets the enemy spell controller search for one immediately productive rain cast. The first pass ignores DryGround, distance to the herd, young-grass growth, and long-term planning: it considers only mature grass that has not yet attempted spreading inside the map-clipped enemy-base area expanded by 30 tiles on every side, and chooses a 5x5 center inside that same contour predicting the largest number of unique new grass cells. The orange contour remains visible for inspection, and every successful enemy cast leaves its blue 5x5 result outline visible for four active real-time seconds; opening the paused in-game menu freezes the remaining outline time. The controller also records the registered-grass count immediately before and after the synchronous rain application so F5/F8 can compare predicted and actual new cells. Egg eater production, other enemy spells, base attacks, and dynamic enemy flag placement are not implemented yet.
...
The enemy side follows the same simulation-first direction. Every four simulation seconds the enemy AI observes projected population plus normalized average satiety across adult enemy herbivores. It fills its current minute-based herbivore cap with a 3:1 stegosaurus/triceratops mix only while that average remains at least 40%; below 40% and below the cap, production skips the turn. A separate spell controller reads the same completed snapshot and may cast rain even though it does not own population decisions. Its first targeting pass supports the ecosystem indirectly by searching only the visible, map-clipped contour extending 30 tiles from the enemy base and choosing the 5x5 area where currently mature, spread-ready grass can create the most unique new grass cells immediately. Once projected herbivores reach or exceed the cap, predator production continues regardless of herbivore satiety. Tyrannosauruses, pterodactyls, and egg eaters also receive persistent indirect rally objectives around the player base; survival, food, reproduction, hunting, and combat still override those routes. Hatched enemy creatures use the same autonomous systems as player creatures rather than receiving direct unit commands.
...
- F3 grid diagnostics, F4 general text diagnostics, the separate F5 enemy-AI snapshot/rain panel, and F8 CSV performance logging are separate systems; F8 appends per-second enemy-rain search count/time, scanned/mature/spread-ready/productive grass workload, candidate workload, application timing, predicted-versus-actual new grass plus their gap, and the maximum single-search duration and best predicted spread seen in each sample.
...
- `scripts/enemies/enemy_energy.gd` starts at 3000 energy, gains catalog-defined income from living enemy creatures, and spends only after a real egg is created;
...
- `scripts/enemies/enemy_spell_controller.gd` listens to the completed enemy-AI snapshot and, whenever adult enemy herbivore average satiety is below its threshold, searches registered mature spread-ready grass inside the map-clipped contour extending 30 tiles from the 2x2 enemy base for the best immediate 5x5 rain result; grass spawn cells and the complete 5x5 candidate square must also stay inside that contour. It ignores DryGround and herd distance in this first pass, spends 50 enemy energy only after finding a positive target, refunds the cost if the shared world rain still fails, draws the persistent orange search contour plus a four-real-second blue 5x5 result outline, and publishes current/session search and rain-application diagnostics to F5 and `PerformanceStats`;
...
- the current production turn handles herbivore and predator eggs, while the separate spell controller handles the first automatic rain; tactical attack planning, base damage, additional enemy spells, dynamic flags, and egg-eater production do not run yet.
...
- every four simulation seconds, `enemy_ai_controller.gd` scans the stable `creatures` and `eggs` groups, filters ownership through `CreatureFaction`, verifies enemy adult creature resources against the enemy catalog, builds one six-species enemy population snapshot, and then performs one production attempt;
...
- the snapshot stores adult, egg, and projected totals separately; eggs affect population planning but are explicitly excluded from the adult-herbivore satiety average; F5 displays that normalized average, the number of adult enemy herbivores included, and whether the 40% herbivore-production gate is active;
