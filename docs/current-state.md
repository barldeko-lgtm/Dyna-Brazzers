...
- a separate enemy spell controller listening to that four-second snapshot; below the adult-enemy-herbivore satiety threshold it scans only registered mature grass whose spread attempt is still available, builds rain-center candidates only around that grass, scores each 5x5 cast by unique immediately spawnable neighbour cells, and spends 50 enemy energy on the best positive target;
...
- `scripts/enemies/enemy_spell_controller.gd` listens to the completed enemy-AI snapshot and, whenever adult enemy herbivore average satiety is below its threshold, searches registered mature spread-ready grass for the best immediate 5x5 rain result; it ignores DryGround and distance in this first pass, spends 50 enemy energy only after finding a positive target, refunds the cost if the shared world rain still fails, and publishes current/session search and rain-application diagnostics to F5 and `PerformanceStats`;
