...
- `scripts/creatures/behaviors/creature_movement_controller.gd` owns grid-route mutation and the public indirect-order boundary; active player flags request route apply/pause/cancel operations through creature methods instead of writing movement or FSM fields directly;
