...
- an active enemy-AI production controller that takes one lightweight population snapshot every four simulation seconds, counts every enemy egg immediately as one future adult, measures normalized average satiety across adult enemy herbivores only, fills a time-based stegosaurus/triceratops cap at a projected 3:1 ratio while that average is at least 50%, and switches to the current predator priority (two raptors, then the first tyrannosaurus, the first pterodactyl, and alternating tyrannosaurus/pterodactyl eggs) when the cap is full or food pressure drops the average below 50%;
...
The enemy side follows the same simulation-first direction. Every four simulation seconds the enemy AI observes projected population plus normalized average satiety across adult enemy herbivores. It fills its current minute-based herbivore cap with a 3:1 stegosaurus/triceratops mix only while that average remains at least 50%; when food pressure lowers it below 50%, herbivore production is blocked and the controller moves to its current alternative predator task until a future rain task is added. Enemy energy is spent only after successful placement. Hatched enemy creatures still use the same autonomous survival, food, reproduction, movement, and combat systems as player creatures rather than receiving direct unit commands.
...
- the snapshot stores adult, egg, and projected totals separately; eggs affect population planning but are explicitly excluded from the adult-herbivore satiety average; F5 displays that normalized average and whether the 50% herbivore-production gate is active;
- while projected herbivores are below the current cap and normalized average adult-herbivore satiety is at least 50%, the selector chooses stegosaurus until there are three projected stegosauruses for the next projected triceratops, then chooses triceratops; if at least one adult herbivore exists and that average drops below 50%, herbivore production is blocked and the current alternative predator task runs instead; that task first ensures two projected raptors, then one projected tyrannosaurus and one projected pterodactyl, then alternates successful tyrannosaurus/pterodactyl purchases; with no adult herbivores the gate remains open so a fresh ecosystem can start; if energy or placement is unavailable, the AI waits without spending;
