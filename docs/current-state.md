...
When no higher-priority behaviour or indirect objective is active, a creature repeatedly chooses between a short idle and a short random walk. After every completed random wander step, movement leaves one stationary physics frame before another step may start, allowing food, reproduction, hunting, combat, and strategic-objective checks to take priority immediately. Exact idle/walk chances and durations remain movement-controller tuning values.
...
Long indirect-order routes are planned against terrain and persistent blockers rather than temporary creature positions. If the next real step is occupied, indirect player-flag and enemy-rally routes first try a short rejoin around the obstruction, then a full alternate route to the same destination. A local detour reconnects at its furthest actual intersection with the preserved route so an earlier nominated rejoin cannot append an already crossed tail. Rejoin selection softly prefers smoother connections, but a viable sharper connection remains an allowed fallback so route aesthetics alone cannot make creatures wait behind temporary occupancy. A successful temporary detour is checked once after a stable per-creature delay of three to five simulation seconds and is replaced only when a static route to the same destination saves meaningful travel cost. When neither route exists, the creature keeps its goal and queued path, waits while the blocking footprint remains unchanged, and retries immediately after that occupancy changes.
...
Defender raptors keep their resource-defined combat stats while they are inside the configured guard radius around their own faction base. Outside that base zone, `creature.gd` applies the configured attack and defense penalties. A shield marker above the raptor shows when the base-guard buff is active; player and enemy raptors share the same rule.
...
The visible 300-pixel right-side HUD uses the supplied `right_panel_top.png` and `right_panel_bottom.png` as two whole-image stretched backgrounds rather than nine-patch frames. At the base 1366x768 viewport the upper minimap/counter art occupies 300x401; the lower menu art starts at Y=401 and stretches through the remaining 300x367 region. The lower texture remains bottom-anchored so taller expanded viewports extend the menu background without changing gameplay layout.
...
- F3 — world grid, paths, occupancy, and selected-creature flag information, including the current route source and the reason for its latest replacement;
- F4 — general text diagnostics;
- F5 — enemy strategic AI, combat reserve, and enemy rain diagnostics;
- F8 — performance CSV recording, including simulation-time alignment and local-rejoin quality counters.
...
The startup screen plays `assets/audio/music/start_menu_theme.mp3` through the Music bus with looping enabled and the same saved music-volume control. Entering gameplay switches to the gameplay theme; both tracks use the shared fade path and a fixed 0.5 linear pre-gain (-6.02 dB), with the user Music slider applied on top. Music and one-shot effects continue independently of simulation speed. Opening the in-game menu does not interrupt music or audio fades.
...
The startup screen provides New Game, Continue, autosave plus three-slot Load, Settings, and Exit. New Game opens a four-entry level list with simple numbered labels; levels 1 and 2 are available, while levels 3 and 4 are visible disabled placeholders. Continue sits directly below New Game, loads the newest valid timestamp across autosave and all manual slots, prefers autosave on equal-second ties, and remains visible but disabled when no valid save exists. The five-button main panel is shifted 150 pixels below screen centre and uses the supplied 720x840 `start_menu_frame.png` at a fixed 370x432 display size. Main buttons are 250x55, centred with 5-pixel separation and shifted up 10 pixels inside the frame. Their source textures remain at 450x109 and the startup-only `StyleBoxTexture` resources scale the whole image into each control, without nine-patch regions; the project's `canvas_items` stretch mode scales the resulting UI for larger display resolutions. All production buttons use the supplied normal, hover/pressed, and disabled stone-frame textures through shared `StyleBoxTexture` resources that scale each whole image into the current control; compact speed controls retain separate content padding without changing their geometry. Startup buttons and headings use Philosopher Bold; secondary labels use Philosopher Regular. The same Bold/Regular contract is applied to player-facing HUD, nature/egg/flag and save menus, creature information, and the result overlay through `assets/ui/dyna_player_ui_theme.tres`; developer F3-F8 controls retain their existing debug typography. The empty status label remains hidden in the main state. The in-game `MENU` button provides Save, autosave plus three-slot Load, Settings, Main Menu, Close Game, and Back.
