...
- quality- and route-aware grass targeting that builds current routes to eight eligible 2x2 pasture candidates and scores each reachable patch by total food value minus two points per path step;
...
- enemy energy starting at 5000, generated by living enemy dinosaurs and spent on enemy-base eggs;
...
The enemy side follows the same simulation-first direction. The current temporary controller creates one affordable egg every five seconds in a fixed six-species order. Hatched enemy creatures use the same autonomous survival, food, reproduction, movement, and combat systems as player creatures rather than receiving direct unit commands.
...
- `scripts/enemies/enemy_egg_production_controller.gd` attempts one species every five seconds in the order stegosaurus, triceratops, egg eater, raptor, pterodactyl, tyrannosaurus;
...
- every five seconds the temporary controller attempts the next catalog species;
