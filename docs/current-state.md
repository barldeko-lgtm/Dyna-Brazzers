...
- an active enemy-AI production controller that takes one lightweight population snapshot every four simulation seconds, counts every enemy egg immediately as one future adult, fills a time-based herbivore cap with stegosaurus/triceratops at a projected 3:1 ratio, and switches to balanced raptor/pterodactyl/tyrannosaurus production once that cap is full;
...
- a separate compact F5 enemy-AI debug panel positioned immediately left of the right-side HUD, using font size 13 and showing AI game time, enemy energy, current production phase, herbivore cap, action, adult counts, egg counts, and projected per-species population;
...
The temporary five-second round-robin producer remains disabled. Every four simulation seconds the strategic AI snapshots projected population. Its herbivore cap is 10 through minute 10, then equals the number of completed game minutes from minute 11 onward, capped at 60. Below the cap it buys stegosaurus/triceratops eggs toward 3:1; at or above the cap it balances raptor, pterodactyl, and tyrannosaurus counts. Egg eater production, flags, spells, and attacks are not implemented yet.
...
The enemy side follows the same simulation-first direction. Every four simulation seconds the enemy AI observes projected population, first fills its current minute-based herbivore cap with a 3:1 stegosaurus/triceratops mix, then balances the three predator species, and spends enemy energy only after successful placement. Hatched enemy creatures still use the same autonomous survival, food, reproduction, movement, and combat systems as player creatures rather than receiving direct unit commands.
...
- `CreatureSpeciesData` stores biological species data, `CreatureFaction` stores validated runtime ownership, `PlayerSpeciesCatalog` stores player-only economy and flag presentation, and `EnemySpeciesCatalog` selects the enemy biological variants without owning strategic rules; `enemy_ai_controller.gd` owns the four-second population snapshot, saved AI game time, minute-based herbivore cap, herbivore ratio choice, predator balancing, and spending turn;
...
- `scripts/enemies/enemy_ai_controller.gd` takes a new snapshot every four simulation seconds, counts enemy adults plus enemy eggs by the shared biological `species_id`, fills a `clamp(full minutes, 10, 60)` herbivore cap at a projected 3:1 stegosaurus/triceratops ratio, then balances raptor/pterodactyl/tyrannosaurus counts, and exposes the latest result through the stable `enemy_ai` group;
...
- the snapshot stores adult, egg, and projected totals separately so F5 can verify the calculation; both the herbivore cap and predator balancing use projected totals, including eggs;
- while projected herbivores are below the current cap, the selector chooses stegosaurus until there are three projected stegosauruses for the next projected triceratops, then chooses triceratops; once the cap is reached it chooses the least represented of raptor, pterodactyl, and tyrannosaurus, using that stable order to break ties; if energy or placement is unavailable, the AI waits without spending;
