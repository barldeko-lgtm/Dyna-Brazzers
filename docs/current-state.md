...
- a fixed 2x2 enemy base with its own 512x512 transparent sprite; it is displayed at 256x256, blocks the world, and automatically creates enemy eggs through the temporary five-second round-robin producer;
...
- enemy energy starting at 5000 and generated by living enemy dinosaurs; the active temporary producer spends it on enemy-base eggs after successful placement;
- a first enemy-AI controller that takes one lightweight population snapshot every four simulation seconds, counts enemy adults by biological `species_id`, and counts every enemy egg immediately as one future adult of its hatch species; its current turn is observation-only and does not yet choose gameplay actions;
...
- a separate F5 enemy-AI debug panel showing the latest four-second turn, adult counts, egg counts, and projected per-species population used by future decisions;
...
Roadmap block `0.5 — Visuals and game interface` is complete. Work on `0.6 — Carnivores and species variety` includes triceratops, tyrannosaurus, raptor, pterodactyl, egg-eater behaviour, shared carnivore/combat prototype code, and the fixed player-base foundation. Roadmap block `0.7 — Player expansion and atmosphere` includes the interactive terrain minimap, player egg creation, the first all-species flag pass, and the first gameplay-audio foundation. Roadmap block `0.8 — Enemies` has started with the two-base foundation, validated three-faction ownership, an enemy six-species catalog, separate enemy biological resources, mirrored enemy energy, an active temporary deterministic egg-production scaffold, and the first four-second enemy-AI population-snapshot loop.

Automatic adult carnivore spawning remains disabled. Automatic enemy-base egg production is still active through the temporary deterministic round-robin scaffold. The first strategic-AI foundation now observes enemy population every four simulation seconds, but population choice, flags, spells, attacks, and spending decisions are not implemented yet.
...
The enemy side follows the same simulation-first direction. The temporary controller actively produces eggs in its fixed six-species round-robin order. A separate first-pass enemy AI now observes the resulting population every four simulation seconds without directly controlling creatures or changing gameplay yet. Hatched enemy creatures use the same autonomous survival, food, reproduction, movement, and combat systems as player creatures rather than receiving direct unit commands.
...
- `CreatureSpeciesData` stores biological species data, `CreatureFaction` stores validated runtime ownership, `PlayerSpeciesCatalog` stores player-only economy and flag presentation, and `EnemySpeciesCatalog` selects the enemy biological variants without owning strategic rules; `enemy_ai_controller.gd` owns the first four-second population snapshot and future strategic turns;
...
- `scripts/enemies/enemy_egg_production_controller.gd` actively uses the five-second species order stegosaurus, triceratops, egg eater, raptor, pterodactyl, tyrannosaurus;
- `scripts/enemies/enemy_ai_controller.gd` takes a new snapshot every four simulation seconds, counts enemy adults plus enemy eggs by the shared biological `species_id`, and exposes the latest snapshot through the stable `enemy_ai` group;
- `scenes/debug/enemy_ai_debug_overlay.tscn` displays that snapshot only when F5 is enabled, without adding enemy-AI details to the general F4 panel;
- insufficient energy or no free egg footprint leaves the current species pending for the next five-second attempt;
- the current AI turn is deliberately observation-only; population balancing, tactical orders, attack planning, base targeting, flags, spells, and strategic spending do not run yet.
...
Enemy egg production and first AI snapshot:

- the enemy base uses the same safe nearby egg-placement method as the player base;
- the temporary controller automatically tries the next catalog species every five seconds;
...
- every four simulation seconds, `enemy_ai_controller.gd` scans the stable `creatures` and `eggs` groups, filters ownership through `CreatureFaction`, and builds one six-species enemy population snapshot;
...
- the snapshot stores adult, egg, and projected totals separately so F5 can verify the calculation while future decisions use the projected total;
