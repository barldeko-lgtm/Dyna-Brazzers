...
Defender raptors may actively protect an allied herbivore from a predator of the opposing faction while not critically hungry. Attacker-role tyrannosaurus and pterodactyl never switch targets or search for a fight to protect an herbivore, but they may continue pursuing an already selected opposing predator after that target starts such a duel. The first eligible protector to reach the attacker reserves the intervention, waits for the next scheduled duel hit to finish, then replaces the herbivore in a new one-on-one duel. The protected herbivore leaves combat, and the replacement duel cannot be interrupted again. Predator-versus-predator duels are never eligible for this protection.
