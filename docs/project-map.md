...
- `scripts/ui/player_ui.gd` — script on `player_hud.tscn`'s right-side panel; handles interactive terrain minimap generation, diet/faction markers, separate player/enemy creature and egg counters, camera viewport display/click navigation, time controls, and egg-controller bootstrap.
...
- `scripts/enemies/enemy_egg_production_controller.gd` — active temporary five-second round-robin enemy egg producer; strategic AI will replace this deterministic scaffold later.
