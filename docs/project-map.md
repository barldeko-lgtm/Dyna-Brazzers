...
- `scripts/ui/player_ui.gd` — interactive terrain minimap generation, camera viewport display and click navigation, creature/egg counters, and time-speed controls.
...
The terrain minimap reads these source ids directly from the active `Ground` TileMapLayer, generates its display texture at runtime, overlays the active camera viewport, and translates minimap clicks into observer-camera world positions.
...
- Terrain minimap generation, camera-frame updates, minimap click navigation, counters, and speed controls belong in `player_ui.gd`.
