...
- `scripts/enemies/enemy_spell_controller.gd` — separate enemy spell decision owner; listens to the completed four-second AI snapshot, triggers rain below the adult-herbivore satiety threshold, scans only registered mature grass with an unused spread attempt, builds candidate centers around those sources, scores each 5x5 cast by unique immediately spawnable grass cells, spends 50 enemy energy after choosing a positive target, and exposes lightweight last-search statistics for testing.
