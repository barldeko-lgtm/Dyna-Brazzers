...
- `scripts/enemies/enemy_spell_controller.gd` — separate enemy spell decision owner; listens to the completed four-second AI snapshot, triggers rain below the adult-herbivore satiety threshold, scans only registered mature grass with an unused spread attempt, builds candidate centers around those sources, scores each 5x5 cast by unique immediately spawnable grass cells, spends 50 enemy energy after choosing a positive target, and exposes the latest scan workload, predicted result, search/apply duration, session maxima, and run counts for F5/F8 diagnostics.
...
- `scripts/debug/performance_stats.gd` — runtime counters and F8 CSV logging, including separate player-flag columns plus enemy-rain search count, total/max search time, grass/candidate workload, best predicted spread, rain-application count/time, and cast rate.
