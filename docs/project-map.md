...
- `scripts/creatures/behaviors/creature_grazing_logic.gd` — herbivore search over ten eligible 2x2 pasture candidates, two-second current-route validation, five-second alternative comparison, staged 80/150/300-tile path limits, upper-bound early stopping, final `food value - path steps * 2` ranking, and movement-controller-owned grazing routes.
