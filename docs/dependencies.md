...
- `enemy_spell_controller.gd` listens to `enemy_ai_controller.gd`'s completed snapshot but does not own population choices. Whenever adult enemy herbivore average satiety is below the snapshot threshold, it may make one rain search and cast independently of whether the production phase skipped or created a predator egg. The current pass first resolves a map-clipped rectangle extending 30 tiles from all sides of the 2x2 enemy-base footprint, rejects grass sources and future grass cells outside it, and rejects any center whose complete 5x5 rain square would cross it, continuously draws that orange contour, and leaves the successful blue 5x5 cast outline for four active real-time seconds and freezes its countdown while the paused in-game menu stops SceneTree processing; each actual target search publishes its latest workload/result plus search/application durations, session maxima, predicted new grass, the immediate before/after registered-grass delta, and total run counts without changing spell decisions;
...
- enemy rain diagnostics are non-blocking world-space drawing owned by `EnemySpellController`: the orange search contour persists, the blue result contour uses the shared rain radius and expires after four active real-time seconds counted only while the pausable controller processes; the in-game menu therefore freezes it, simulation speed does not shorten it, and neither contour is serialized or allowed to alter terrain, targeting, occupancy, or spell cost;
...
- F5 may read only the public `enemy_ai` snapshot/timing/energy/cap/satiety fields and the public `enemy_spell_controller.get_rain_debug_data()` result; it must not make strategic decisions or mutate enemy state. Its satiety line should identify the value as enemy-only and show the included adult-enemy-herbivore count, while rain lines remain diagnostic-only and may show the configured search radius plus result-outline duration;
...
- the first rain-targeting pass must ignore DryGround value, young-grass growth, herd distance, and long-term recovery. Before scanning it must resolve the current `EnemyBase.anchor_tile` plus `footprint_size`, expand that footprint by the exported 30-tile radius on every side, and clip the result to `world_grid.map_min`/`map_max`. It may scan `world_grid.grass_by_tile`, but only stage-4 sources and cardinal spawn cells inside that contour may enter the score, and a candidate center is valid only when its complete shared-radius 5x5 square remains inside the contour. Retained grass must have `has_tried_to_spread == false` and at least one cardinal cell accepted by the same `can_host_grass()` plus `has_grass_at_tile()` rules used by real spreading; each unique immediately spawnable cell counts once;
...
- `PerformanceStats` writes `flag_creatures_scanned_per_sec`, `flag_path_requests_per_sec`, and `flag_path_failures_per_sec` to new F8 CSV logs; it also appends enemy-rain search count, total milliseconds per real second, maximum single-search milliseconds, scanned/spread-ready/productive grass, unique spawn targets, candidate centers, best predicted spread, actual registered-grass increase, prediction gap, rain-application timing, and cast rate;
...
- notifying the shared 2x2 grazing-pasture cache after registration, removal, or a food-stage change.
...
- every registration, unregistration, or stage change that alters edible grass must refresh the four overlapping 2x2 pasture-cache anchors.
